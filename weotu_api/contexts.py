...
class Ctx(conv.State):
    ...
    def __init__(self, req = None):
        if req is not None:
            self.req = req
            ctx_env = req.environ.get('weotu-api', {})
    ...
    def blank_req(self, path, environ = None, base_url = None, headers = None, POST = None, **kw):
        ...
        ctx_env = env.setdefault('weotu-api', {})
    ...
    def lang_del(self):
        del self._lang
        if self.req is not None and self.req.environ.get('weotu-api') is not None \
                and '_lang' in self.req.environ['weotu-api']:
            del self.req.environ['weotu-api']['_lang']

    def lang_get(self):
        if self._lang is None:
            self._lang = ['en-US', 'en']
            if self.req is not None:
                self.req.environ.setdefault('weotu-api', {})['_lang'] = self._lang
        return self._lang
    ...
    def lang_set(self, lang):
        ...
        if self._translator is not None:
            ...
            if self.req is not None and self.req.environ.get('weotu-api') is not None \
                    and '_translator' in self.req.environ['weotu-api']:
                del self.req.environ['weotu-api']['_translator']
